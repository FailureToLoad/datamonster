...
func main() {
	...
	app := NewServer(keycloakConfig, sessions, settlementController, survivorController)
	app.Run()
}
...
func NewServer(keycloak *auth.KeycloakConfig, sessions *valkey.SessionStore, settlements *settlement.Controller,
	...
	keycloak.RegisterRoutes(sessions, router)
	router.Mount("/api", protectedRoutes(sessions, settlements, survivors))
	return Server{
		Mux: router,
	}
}

func protectedRoutes(sessions *valkey.SessionStore, settlement *settlement.Controller, survivor *survivor.Controller) http.Handler {
	r := chi.NewRouter()
	...
	r.Use(SecureOptions())
	r.Use(CacheControl)
	r.Use(auth.AuthMiddleware(sessions))
