...
func (k *KeycloakConfig) LoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	authURL := k.OAuth2Config.AuthCodeURL(state)
...
func (k *KeycloakConfig) LogoutHandler(sessions *valkey.SessionStore) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		logoutURL := k.IssuerURL + "/protocol/openid-connect/logout"
		fullLogoutURL := logoutURL + "?post_logout_redirect_uri=" + postLogoutRedirect + "&client_id=" + k.ClientID
		http.Redirect(w, r, fullLogoutURL, http.StatusTemporaryRedirect)
	}
}

func (k *KeycloakConfig) RegisterRoutes(sessions *valkey.SessionStore, r interface {
	Get(string, http.HandlerFunc)
	Post(string, http.HandlerFunc)
},
) {
	r.Get("/auth/login", k.LoginHandler)
	r.Get("/auth/callback", k.CallbackHandler(sessions))
	r.Get("/auth/logout", k.LogoutHandler(sessions))
	r.Get("/auth/check", CheckHandler(sessions))
}
